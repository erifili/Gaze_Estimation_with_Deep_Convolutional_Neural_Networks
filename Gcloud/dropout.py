...
def readData(filename, flag):
    ...
    if flag == 'Training':
        ...
        test_face = data['face']
        test_left_eye = data['leftEye']
        test_right_eye = data['rightEye']
        test_face_grid = data['face_grid']
...
def model(train_data, val_data, net_params, epochs, eta, train_data_size, val_data_size, batch_size, hold_p, min_diff, patience, print_per_epoch, save_model_dir_fname):
    ...
    #-------> face
    # dropout layer for face
    fc1_f = tf.nn.relu(fully_conn_layer(pooling4_f_flat, W['fc1_f'], b['fc1_f']))
    fc2_f = tf.nn.relu(fully_conn_layer(fc1_f, W['fc2_f'], b['fc2_f']))

    #-------> face grid
    drop_out_fg = tf.nn.dropout(face_grid, keep_prob = hold_prob)
    fc1_fg = tf.nn.relu(fully_conn_layer(drop_out_fg, W['fc1_fg'], b['fc1_fg']))
    fc2_fg = tf.nn.relu(fully_conn_layer(fc1_fg, W['fc2_fg'], b['fc2_fg']))

    ## output
    fc = tf.concat([fc_e1, fc2_f, fc2_fg], 1)
    # dropout layer for FC1
    drop_out_FC1 = tf.nn.dropout(fc, keep_prob = hold_prob)
    fc1 = tf.nn.relu(fully_conn_layer(drop_out_FC1, W['fc1'], b['fc1']))
    ...
    with tf.Session() as sess:
        ...
        for ep in tqdm(range(1, epochs + 1)):
            ...
            for batch in readChunks(train_data, train_data_size, batch_size):
                ...
                sess.run(optimizer, feed_dict={face: face_batch, left_e:left_eye, right_e:right_eye, face_grid: faceGrid_batch,  y: y_batch, hold_prob: hold_p})
                                
                train_batch_loss, train_batch_error = sess.run([loss, error], feed_dict={face: face_batch, left_e:left_eye, right_e:right_eye, face_grid: faceGrid_batch,  y: y_batch, hold_prob: hold_p})
            ...
            for batch in readChunks(val_data, val_data_size, batch_size):
                ...
                y_batch_val = batch[4]
                val_batch_loss, val_batch_error = sess.run([loss, error], feed_dict={face: face_batch_val, left_e:left_eye_val, right_e:right_eye_val, face_grid: faceGrid_batch_val,  y: y_batch_val, hold_prob: hold_p})
...
def main():
    ...
    if args.tr:
        ...
        for batch in readChunks(test_data, test_data[0].shape[0], batch_size):
            ...
            y_pred = sess.run(pred, feed_dict={face_test: face_batch_test, left_e_test:left_eye_test, right_e_test:right_eye_test, face_grid_test: faceGrid_batch_test,  y: y_batch_test,  hold_prob: hold_p})
