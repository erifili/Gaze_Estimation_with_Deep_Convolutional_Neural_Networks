...
def readData(filename, flag):
    ...
    if flag == 'Training':
        ...
        train_data = [train_left_eye, train_right_eye, train_face_grid, train_labels]
        val_data = [val_left_eye, val_right_eye, val_face_grid, val_labels]
        ...
        test_data = [test_left_eye, test_right_eye, test_face_grid, test_labels]
    
        return test_data

def readChunks(file, data_size, batch):
    for i in range(0, data_size, batch):
        left_eye = file[0][i:i+batch]
        right_eye = file[1][i:i+batch]
        face_grid = file[2][i:i+batch]
        labels = file[3][i:i+batch]

        data = [left_eye, right_eye, face_grid, labels]
        
        yield data
    
def prepare_data(data):
    left_eye, right_eye, face_grid, labels = data
    ...
    d = [left_eye, right_eye, face_grid, labels]
    return d
...
def model(train_data, val_data, net_params, epochs, eta, train_data_size, val_data_size, batch_size, min_diff, patience, print_per_epoch, save_model_dir_fname):
    ...
    W = {
            ...
            'fc1': init_weights([net_params.fc_e1_size + net_params.fc_fg2_size, net_params.fc1_size], name='fc1_w'),
    ...
    fc_e1 = tf.nn.relu(fully_conn_layer(fc_e1_in, W['fc_e1'], b['fc_e1']))

    #-------> face grid
    fc1_fg = tf.nn.relu(fully_conn_layer(face_grid, W['fc1_fg'], b['fc1_fg']))
    fc2_fg = tf.nn.relu(fully_conn_layer(fc1_fg, W['fc2_fg'], b['fc2_fg']))

    ## output
    fc = tf.concat([fc_e1, fc2_fg], 1)
    fc1 = tf.nn.relu(fully_conn_layer(fc, W['fc1'], b['fc1']))
    ...
    with tf.Session() as sess:
        ...
        for ep in tqdm(range(1, epochs + 1)):
            ...
            for batch in readChunks(train_data, train_data_size, batch_size):
                left_eye = batch[0]
                right_eye = batch[1]
                faceGrid_batch = batch[2]
                y_batch = batch[3]
                
                sess.run(optimizer, feed_dict={left_e:left_eye, right_e:right_eye, face_grid: faceGrid_batch,  y: y_batch})
                                
                train_batch_loss, train_batch_error = sess.run([loss, error], feed_dict={left_e:left_eye, right_e:right_eye, face_grid: faceGrid_batch,  y: y_batch})
            ...
            for batch in readChunks(val_data, val_data_size, batch_size):
                left_eye_val = batch[0]
                right_eye_val = batch[1]
                faceGrid_batch_val = batch[2]
                y_batch_val = batch[3]
                val_batch_loss, val_batch_error = sess.run([loss, error], feed_dict={left_e:left_eye_val, right_e:right_eye_val, face_grid: faceGrid_batch_val,  y: y_batch_val})
...
def main():
    ...
    if args.tr:
        ...
        for batch in readChunks(test_data, test_data[0].shape[0], batch_size):
            left_eye_test = batch[0]
            right_eye_test = batch[1]
            faceGrid_batch_test = batch[2]
            y_batch_test = batch[3]
            y_pred = sess.run(pred, feed_dict={left_e_test:left_eye_test, right_e_test:right_eye_test, face_grid_test: faceGrid_batch_test,  y: y_batch_test})
