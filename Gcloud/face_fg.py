...
def readData(filename, flag):
    ...
    if flag == 'Training':
        ...
        test_data = [test_face, test_face_grid, test_labels]
...
def readChunks(file, data_size, batch):
    for i in range(0, data_size, batch):
        ...
        face_grid = file[1][i:i+batch]
        labels = file[2][i:i+batch]

        data = [face, face_grid, labels]
...
def model(train_data, val_data, net_params, epochs, eta, train_data_size, val_data_size, batch_size, min_diff, patience, print_per_epoch, save_model_dir_fname):
    ...
    W = {
            ...
            'fc1': init_weights([net_params.fc_f2_size + net_params.fc_fg2_size, net_params.fc1_size], name='fc1_w'),
    ...
    with tf.Session() as sess:
        ...
        for ep in tqdm(range(1, epochs + 1)):
            ...
            for batch in readChunks(val_data, val_data_size, batch_size):
                ...
                faceGrid_batch_val = batch[1]
                y_batch_val = batch[2]
                val_batch_loss, val_batch_error = sess.run([loss, error], feed_dict={face: face_batch_val, face_grid: faceGrid_batch_val,  y: y_batch_val})
...
def main():
    ...
    if args.tr:
        ...
        for batch in readChunks(test_data, test_data[0].shape[0], batch_size):
            ...
            faceGrid_batch_test = batch[1]
            y_batch_test = batch[2]
            y_pred = sess.run(pred, feed_dict={face_test: face_batch_test, face_grid_test: faceGrid_batch_test,  y: y_batch_test})
